...
const PLACEHOLDER = makeTile(0, 0); // not rendered; just satisfies the type
        ...
        >
            <div className="flex flex-col">
                ...
                <span className="text-[10px] uppercase tracking-wider text-pr-ivory-dim">
                    Team {seat.team} · {handCount} tile{handCount === 1 ? "" : "s"}
                    {isCurrentTurn ? (isAiThinking ? " · thinking…" : " · playing") : ""}
                </span>
            </div>
            <div className="flex items-center gap-0.5">
                ...
                {handCount === 1 && (
                    <span className="ml-1 text-[10px] font-bold text-pr-coqui">IMMUNE</span>
                )}
            </div>
        </div>
    );
